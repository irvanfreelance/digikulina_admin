...
export async function POST(request: Request) {
  try {
    ...
    const query = `INSERT INTO "customers" (${columnsString}) VALUES (${placeholders}) RETURNING *`;
    const { rows } = await pool.query(query, values);
...
export async function PUT(request: Request) {
  try {
    ...
    const query = `UPDATE "customers" SET ${setString} WHERE id = $${values.length} RETURNING *`;
    const { rows } = await pool.query(query, values);
