...
const MODULE_TABS = {
  katalog: ['products', 'categories', 'modifier_groups', 'modifier_options'],
  inventory: ['branch_products', 'branch_modifier_options', 'stock_ledgers'],
  sales: ['orders', 'order_items', 'reservations', 'order_reviews'],
  hr_crm: ['customers', 'coupons', 'users', 'cashier_shifts'],
  settings: ['branches', 'table_areas', 'tables', 'tax_configs', 'kds_stations', 'payment_logs', 'notif_logs']
};

const TABLE_CONFIGS_STRING = `const TABLE_CONFIGS: any = {
...
const template = (moduleName, tabs) => `"use client";
...
export default function ${moduleName.replace('_', '')}Page() {
...
Object.entries(MODULE_TABS).forEach(([moduleName, tabs]) => {
  const modDir = path.join(adminDir, moduleName);
  if (!fs.existsSync(modDir)) {
    fs.mkdirSync(modDir, { recursive: true });
  }
  fs.writeFileSync(path.join(modDir, 'page.tsx'), template(moduleName, tabs), 'utf8');
});

console.log('Regenerated explicit pages for admin modules with form fixes.');
